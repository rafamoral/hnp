...
function(object, sim=99, conf=.95, resid.type, maxit,
                ...
                newclass=F, diagfun, simfun, fitfun, data, ...) {
if(!missing(data)) {
  if(class(data)!="data.frame") stop("The dataset provided must be of class 'data.frame'")
  }
...
if(newclass) {
  ...
  stop("When running hnp for a non-implemented class or diagnostic measure, you must provide functions for arguments:", "\n", 
    ...
    if(missing(diagfun)) diagfun <- function(obj) resid(obj)
    fam <- "newfam"}
  } else {
if(class(object)[1]!="numeric"&class(object)[1]!="aov"&class(object)[1]!="lm"&class(object)[1]!="lmerMod"&
   class(object)[1]!="aovList"&class(object)[1]!="glm"&class(object)[1]!="negbin"&
   class(object)[1]!="vglm"&class(object)[1]!="aodml"&class(object)[1]!="zeroinfl"&
   class(object)[1]!="hurdle"&class(object)[1]!="multinom"&class(object)[1]!="glmerMod"&
   class(object)[1]!="aovlist"&class(object)[1]!="gamlss"&class(object)[1]!="glmmadmb"&
   class(object)[1]!="integer") {
   stop("This function has not been implemented for objects of class '", class(object)[1], "'. If you wish to supply your own fitting, simulation and diagnostic extration codes, check the package vignette for detailed instructions.")
   }
if(class(object)[1]=="numeric"|class(object)[1]=="integer") {
  fam <- "norm.res"
  }
if(class(object)[1]=="aov" | class(object)[1]=="lm") {
  ...
  if(class(object)[1]=="aov") fam <- "gaussian.aov"
  if(class(object)[1]=="lm") fam <- "gaussian.lm"
...
# closing the else statement for newclass=T
  }
# Set options to not show warning messages
...
if(fam=="ZIB") {
  ...
  y. <- lapply(rep(length(f.zib), sim), rzibinom, m., f.zib, phi)
  fmla <- as.formula(paste("cbind(y.[[i]],m.-y.[[i]]) ~ ", paste(object@terms$terms)[3]))
  if(halfnormal) {for(i in 1:sim) {
    if(verb.sim) cat("Simulation", i, "out of", sim, "\n") 
...
# Gaussian AOV with Error() term
if(fam=="gaussian.aovlist") {
...
# Gaussian residuals
if(fam=="norm.res") {
  cat("Half-normal plot with simulated envelope generated assuming the residuals are normally distributed under the null hypothesis.", "\n")
  if(scale) {
    ex <- mean(object)
    sdx <- sd(object)
...
if(fam=="newfam") {
  if(missing(data)) stop("Please provide the dataset used in your model fit when using user-defined functions to implement a new class in hnp")
  if(halfnormal) {res <- sort(abs(diagfun(object)))
    } else {res <- sort(diagfun(object))}
  y. <- lapply(rep(length(res), sim), simfun, object)
  if(halfnormal) {for(i in 1:sim) {
  if(verb.sim) cat("Simulation", i, "out of", sim, "\n")
  datax <- data.frame(data, y.=y.[[i]])
  res <- cbind(res,sort(abs(diagfun(fitfun(datax)))))}
    } else {for(i in 1:sim) {
  if(verb.sim) cat("Simulation", i, "out of", sim, "\n")
  datax <- data.frame(data, y.=y.[[i]])
  res <- cbind(res,sort(diagfun(fitfun(datax))))}}    
  }
# Reset options to show warnings
